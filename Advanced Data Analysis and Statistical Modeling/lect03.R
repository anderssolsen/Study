rm(list=ls())
setwd("~/Kurser/kursus02424/2020/lect03")
##################################################
#
##################################################
...
## Including box- cox transformation
loglikMod01 <- logLik(model01) - (sum(0.75*log(ozone.pollution$ozone[-17]))-n*log(0.25))

loglikMod01
logLik(model0)

par(mfrow=c(1,1))
bc <- boxcox(model0,lambda=seq(0,1,by=0.001))
diff(range(bc$y))

loglikMod01-logLik(model0)## Almost identical
