# Code used in Lecture No. 5 - Motivating example with mice.
...
# Predicted values in link domain and in response domain
predict(mice.glm,type='link',se.fit=TRUE)
fit
se.fit
predict(mice.glm,type='response',se.fit=TRUE)

# Residuals in the three ways:
residuals(mice.glm,type="response") #type = deviance, pearson
