rm(list=ls())
setwd("~/Kurser/kursus02424/2018/lectures/lect09/")
library(nlme)
###################################################
...
solve(t(Z)%*%solve(Sigma)%*%Z+solve(Psi))%*%t(Z)%*%
    solve(Sigma)%*%
...
fit.gau <- lme(lnc~month+treatm+month:treatm,
               random=~1|cage,
               correlation=
                   corGaus(form=~as.numeric(month)|cage,
                           nugget=TRUE), data=rats)
##################################################
...
## Compare using "ML"
 ...
 fit.gau<- lme(lnc~month+treatm+month:treatm,
               random=~1|cage,
               correlation=corGaus(form=~as.numeric(month)|cage,
                                   nugget=TRUE),
               data=rats, method="ML")
...
for(i in 1:max(as.numeric(rats$treatm))){
    for(j in 1:max(as.numeric(rats$cage))){
        I <- as.numeric(rats$treatm)==i &
            as.numeric(rats$cage)==j
        for(k in 1:9){
            V[k] <- V[k] +
                sum(diff(residuals(fit.gau)[I],lag=k)^2)
            N[k] <- N[k] +
                length(diff(residuals(fit.gau)[I],lag=k))
        }
    }
}
V
