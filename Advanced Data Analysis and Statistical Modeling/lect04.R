...
## Example (Simulation)
rm(list=ls())
setwd("C:/Users/ander/Documents/10. Semester/Advanced data analysis/Week03")
## Simulate
set.seed(123456)
...
fit11 <- lm(y ~ (sex + tmt + alt)^2 - sex:alt)
anova(fit11,fit1)
##################################################
...
anova(fit11,fit0)
anova(fit11)
summary(fit11) ## Note the difference
...
legend(x="topleft",
       legend=c("F:Placebo", "M:Placebo",
                "F:Treatment","M:Treatment"),
       col=c(1,1,2,2),pch=c(1,2))
lines(sort(alt),predict(fit3,newdata=data.frame(sex="F",tmt=factor(0),
                                                alt=sort(alt))))
lines(sort(alt),predict(fit3,newdata=data.frame(sex="F",tmt=factor(1),
                                                alt=sort(alt))),col=2)
lines(sort(alt),predict(fit3,newdata=data.frame(sex="M",tmt=factor(0),
                                                alt=sort(alt))),
      col=1)
lines(sort(alt),predict(fit3,newdata=data.frame(sex="M",tmt=factor(1),
                                                alt=sort(alt))),
      col=2)


## Forward/backward/both (uses AIC, not p-values)
(fit.step <- step(fit0,~.+1))
(fit.back <- step(fit0,~+1,direction="back"))
(fitNull <- lm(y ~ 1)) ## select null model for forward selection
...
##################################################
## Collinearity and polynomial regression (go back to last week)
setwd("C:/Users/ander/Documents/10. Semester/Advanced data analysis/Week03")
...
## Assuming that data is a timeseries we can check
## independence by
par(mfrow=c(1,1))
acf(rstudent(model3))
